...
public class GamePanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public GamePanel(int aiChoice) {
        ...
        if (aiChoice == 0) {
            aiController = new NoobBot(player);
        } else if (aiChoice == 1) {
            aiController = new SmartBot(player, true, false, false);
        } else {
            aiController = new SmartBot(player, true, false, true);
        }
    ...
    private void tryFireAtComputer(Position mousePosition) {
        ...
        if (!computer.isPositionMarked(targetPosition)) {
            ...
            if (!computer.isAllShipsDestroyed()) {
                doAITurn();
            }
        }
    }
